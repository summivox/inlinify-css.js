...
(function() {
  var CSS_TEXT, MARKER, arrayize, cmp, cmpSpec, getSpec, inlinify;
  ...
  window.getSpec = getSpec = function(x) {
  ...
  window.cmpSpec = cmpSpec = function(a, b) {
    var c, i, _i;
  ...
  window.inlinify = inlinify = function(root, rules) {
    var selected;
    ...
    if ((selected = root.querySelectorAll("[data-" + MARKER + "]")) != null) {
      arrayize(selected).forEach(function(el) {
        var k, p, _base, _ref;
        _ref = el.stylePlus;
        for (k in _ref) {
          p = _ref[k];
          (_base = el.style)[k] || (_base[k] = p.value);
        }
        delete el.stylePlus;
  ...
  $(function() {
    ...
    return window.c = cssParser.parse(cssParser.tokenize(CSS_TEXT));
  });
